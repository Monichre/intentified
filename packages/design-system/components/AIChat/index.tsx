...
// Custom hook to detect mobile - you may need to implement this based on your needs
const useIsMobile = () => {
...
const AIChat = (props: AIChatProps) => {
  ...
  return (
    <div className='w-[600px] h-[380px] mx-auto flex justify-center items-center text-[#c2e6eb8f] text-sm'>
      <motion.div
      ...
      >
        <motion.div
          ...
          className="bg-gradient-to-b from-[rgba(240,254,255,.06)] to-transparent bg-[rgba(194,230,235,.02)] h-3 absolute left-4 right-4 -top-3 backdrop-blur rounded-t-[10px] before:rounded-[inherit] before:content-[''] before:absolute before:inset-0 before:border before:border-[#c2e6eb14] before:border-b-0"
        />
        <motion.div
          initial={{y: -12, opacity: 0}}
          animate={{opacity: state === 0 ? 0 : 1, y: state === 0 ? -12 : 0}}
          transition={{duration: 0.3, ease: 'easeOut', delay: 0.1}}
          className="bg-gradient-to-b from-transparent to-[rgba(240,254,255,.06)] bg-[rgba(194,230,235,.02)] h-3 absolute left-4 right-4 -bottom-3 backdrop-blur rounded-b-[10px] before:rounded-[inherit] before:content-[''] before:absolute before:inset-0 before:border before:border-[#c2e6eb14] before:border-t-0"
        />
        <div className="shadow-[0_0_24px_#b9e7ec14_inset,0_-4px_12px_#b9e7ec0f_inset,0_1px_1px_#b9e7ec1f_inset] bg-[#c2e6eb0a] backdrop-blur-sm overflow-hidden rounded-[10px] relative before:rounded-[inherit] before:content-[''] before:absolute before:pointer-events-none before:inset-0 before:border before:border-[#c2e6eb1f]">
          <div
            className={`p-3 flex items-center gap-2 relative before:content-[''] before:absolute before:inset-0 ${state > 2 ? 'before:border-b before:border-[#c2e6eb12]' : ''}`}
          >
            <div className='text-[#c2e6eb8f] flex'>
              <SearchIcon />
            </div>
            <div
              className={`${state <= 1 ? 'text-[#c2e6eb59]' : 'text-[#c2e6ebd4]'} text-sm leading-5 font-light flex-1`}
            >
              {state <= 1 ? (
                'Ask about anything...'
              ) : (
            ...
            <motion.div
              ...
              className='text-[#c2e6eb59] bg-[#c2e6eb0a] px-1.5 rounded-md text-[13px]'
            >
              ⌘ K
            </motion.div>
          </div>
            ...
            <AnimatePresence mode='popLayout'>
              {state < 5 ? (
                      ...
                      <div className='p-2 relative'>
                        {state >= 4 && (
                          <motion.div
                            ...
                            className='absolute inset-2 h-9 rounded-md bg-[#c2e6eb0a]'
                          />
                        )}
                        {[
                          ...
                          >
                            ...
                            {i === 0 ? (
                              ...
                              <div className='flex flex-col'>
                                ...
                                <div className='text-xs overflow-hidden text-ellipsis whitespace-nowrap w-[260px] sm:w-full'>
                                  {item.description}
                                </div>
                              </div>
                            )}
                ...
                <motion.div
                  ...
                  className='px-[18px] pt-[14px] pb-[18px] h-full flex flex-col'
                >
                  <div className='flex flex-1 items-start gap-4 text-[#c2e6ebd4] text-[13px] [&_svg]:flex-shrink-0 [&_svg]:text-[#c2e6eb] [&_svg]:w-4 [&_svg]:h-4'>
                    <StarsIcon />
                    <div>
                      <Typewriter speed={10} text={description} />
                    </div>
                  </div>
                  <div className="shadow-[0_0_24px_#cad7f914_inset,0_1px_1px_#c9d7f91f_inset] bg-[#c2e6eb0a] h-9 rounded-md flex items-center relative px-3 py-2 before:rounded-[inherit] before:content-[''] before:absolute before:inset-0 before:border before:border-[#c2e6eb1f]">
        ...
        {state == 4 && (
          <motion.div
            ...
            className='absolute top-[110px] right-[30px] z-[4]'
          >
            <ArrowCursorIcon />
          </motion.div>
        )}
...
const TypingCursor = () => {
  return (
    <div className='w-[1px] h-3 inline-block ml-[3px] -mb-[1px] bg-[#c2e6ebd4] animate-blink' />
  )
}
