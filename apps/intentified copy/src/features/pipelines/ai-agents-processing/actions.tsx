 ...
 */
async function parallelProcessing(model: any, content: string, aspects = "") {
  ...
  const tasks = aspectList.map((aspect) => ({
    ...
    icon: aspect.includes("summary") ? "list" : aspect.includes("analysis") ? "search" : "sparkles",
    color: aspect.includes("summary") ? "blue" : aspect.includes("analysis") ? "green" : "purple",
  }))
 ...
 */
async function orchestratorWorker(model: any, requirements: string, constraints = "") {
