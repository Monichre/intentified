...
interface UploadState {
  status: "idle" | "loading" | "uploading" | "processing" | "extracting" | "analyzing" | "error" | "success"
  progress: number
  fileName?: string
  error?: string
  document?: Document
...
function WorkflowStep({ label, status }: WorkflowStepProps) {
  ...
  return (
    <motion.div className="flex flex-col items-center gap-2" layout="position" transition={animations.springTransition}>
      <motion.div
        className={cn(
        ...
        <motion.div key={stage.key} className="flex items-center" layout="position">
          {index > 0 && (
            <motion.div
              className={cn(
                "h-[2px] w-12",
                WORKFLOW_STAGES.findIndex((s) => s.key === currentStage) > index ? "bg-primary" : "bg-muted",
              )}
              layout="preserve-aspect"
              transition={animations.easeTransition}
            />
...
function StateContent({ state, children }: { state: UploadState["status"]; children: React.ReactNode }) {
  return (
    <AnimatePresence mode="wait">
      <motion.div
        key={state}
        ...
        <>
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            <p className="text-sm text-muted-foreground">
              ...
              <span className="text-primary font-medium cursor-pointer hover:underline underline-offset-2">browse</span>
            </p>
    ...
    case "loading":
      return (
        <>
          ...
          <p className="text-sm text-muted-foreground max-w-[16rem]">This may take a few minutes.</p>
        </>
      )
    case "uploading":
      return (
    ...
    case "success":
      return (
        <>
          ...
          <p className="text-sm text-muted-foreground">Your document has been processed successfully.</p>
        </>
      )
  }
}
      ...
      <div {...getRootProps()} className="relative">
        ...
        >
          ...
          >
            ...
            {(isUploading || isProcessing) && <WorkflowProgress currentStage={uploadState.status} />}
