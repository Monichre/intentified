...
export async function analyzeDocument(supabase: SupabaseClient<Database>, documentId: string) {
  try {
    ...
    console.log(`[Analysis] Found extracted text of length: ${document.extracted_text.length}`)
      ...
      if (analysis.entities.length > 0) {
        ...
        await pMap(
          ...
          async (batch, batchIndex) => {
            ...
            const { error: entityError } = await supabase.from("doc_processor_document_entities").insert(
              batch.map((entity) => ({
                document_id: documentId,
            ...
            if (entityError) {
              console.error(`[Analysis] Entity batch ${batchIndex + 1} storage error:`, entityError)
              throw entityError
            }
    ...
    const { error: vectorizeError } = await supabase.from("doc_processor_processing_tasks").insert({
      document_id: documentId,
      task_type: "vectorize",
    ...
    if (vectorizeError) {
      console.error("[Analysis] Vectorization task creation error:", vectorizeError)
