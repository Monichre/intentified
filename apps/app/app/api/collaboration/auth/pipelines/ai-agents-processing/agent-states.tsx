...
import { TextRoll, TextEffect } from "./effects"
...
function LoadingState({ agent, elapsedTime = 0 }: { agent: AgentType; elapsedTime?: number }) {
  // Calculate progress percentage
  const progress = Math.min((elapsedTime / (agent.averageTime || 30)) * 100, 100)
      ...
      <div className="max-w-2xl w-full space-y-8">
        ...
        <motion.div variants={staggerItemVariants} className="space-y-1">
          ...
          >
            ...
            <span className="text-sm font-medium text-neutral-800">Estimated Time</span>
          ...
          >
            ...
            <div className="px-4 sm:px-6 py-3 border-b border-neutral-100 bg-neutral-50/50">
              <div className="flex items-center gap-2">
                ...
                <div className="text-xs sm:text-sm font-medium text-neutral-700">Agent Details</div>
              </div>
            </div>

            {/* Card Content */}
            <div className="p-4 sm:p-6 text-left">
              <motion.div variants={staggerContainerVariants} initial="initial" animate="animate" className="space-y-6">
                {agent.description && (
                  <motion.div variants={staggerItemVariants} className="space-y-2">
                    ...
                    <p className="text-xs sm:text-sm text-neutral-600 leading-relaxed ">{agent.description}</p>
                  </motion.div>
                )}

                {agent.capabilities && (
                  <motion.div variants={staggerItemVariants} className="space-y-3">
                    <h4 className="text-xs font-medium text-neutral-500">Capabilities</h4>
    ...
    >
      <Card className="mt-1 pt-1 p-0 space-y-8 rounded-2xl border-none shadow-[0px_1px_1px_0px_rgba(0,_0,_0,_0.05),_0px_1px_1px_0px_rgba(255,_252,_240,_0.5)_inset,_0px_0px_0px_1px_hsla(0,_0%,_100%,_0.1)_inset,_0px_0px_1px_0px_rgba(28,_27,_26,_0.5)]">
        <div className="p-4 space-y-6">
                ...
                <div className="  flex items-center gap-1  pl-1    px-[2px] py-[2px] ">
                  <div className="flex items-center space-x-2 text-sm text-neutral-500 ">
                    ...
                    <div className="  flex items-center gap-1  pl-1   px-[2px] py-[1px] ">
                      <p className="font-medium text-muted-foreground/60 tracking-wide text-[9px]">Input fields</p>
          ...
          {/* Details */}
          <motion.div key={`${agent.name}-details`} variants={staggerItemVariants} className="space-y-4 text-sm">
            <div className="p-4 bg-neutral-50 rounded-lg shadow-[0px_1px_1px_0px_rgba(0,_0,_0,_0.05),_0px_1px_1px_0px_rgba(255,_252,_240,_0.5)_inset,_0px_0px_0px_1px_hsla(0,_0%,_100%,_0.1)_inset,_0px_0px_1px_0px_rgba(28,_27,_26,_0.5)] space-y-4">
              <div>
                <h4 className="font-medium text-neutral-900 mb-1 text-xs">Best Use Cases</h4>
                <p className="text-neutral-600 text-xs">{agent.context}</p>
              </div>
              <div className="space-y-4 pt-4 border-t border-neutral-200">
                <div>
                  <h4 className="font-medium text-neutral-900 mb-2 text-xs">Inputs</h4>
                    ...
                    {agent.inputFields.map((field, index) => (
                      <div key={index} className="flex items-center gap-2 text-neutral-600">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-neutral-500 mt-0.5">{field.placeholder.split("\n")[0]}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div className="font-medium text-neutral-900 mb-1 text-xs">Output</div>
          ...
          {/* Features */}
          <motion.div key={`${agent.name}-features`} variants={staggerItemVariants} className="space-y-3">
            <h4 className="font-medium text-neutral-900 text-xs">Capabilities</h4>
            <div className="flex flex-wrap gap-2">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-8 py-4 border-t border-neutral-100 rounded-b-2xl bg-[#F7F7F8]">
          <Button variant="ghost" size="sm" className="text-neutral-500 hover:text-neutral-900 text-xs">
...
function CircularProgress({
  ...
  return (
    <div className="relative inline-flex group">
      ...
      {/* Static circle */}
      <svg width={size} height={size} className="transform -rotate-90 rounded-full">
        <circle
          cx={size / 2}
          cy={size / 2}
          r={radius}
      ...
      <div className="absolute inset-0 flex items-center justify-center rounded-full">
        <motion.div variants={pulseVariants} animate="animate" className="rounded-full flex flex-col items-center ">
          {children}
...
function Countdown({
  ...
  return (
    ...
    >
      <Clock02Icon className={cn("w-3.5 h-3.5", loading && "animate-[spin_3s_linear_infinite]")} />
      <NumberFlowGroup>
        <div
        ...
        >
          <div className="flex items-baseline gap-[2px]">
            <NumberFlow
              ...
              className={cn("text-sm transition-colors duration-200", loading ? "text-blue-600" : "text-neutral-600")}
            />
            <span
              className={cn(
          ...
          <span
            className={cn("mx-0.5 transition-colors duration-200", loading ? "text-blue-400/70" : "text-neutral-300")}
          >
            .
          </span>
          <div className="flex items-baseline">
            <NumberFlow
              value={ms}
              format={{ minimumIntegerDigits: 2 }}
              className={cn("text-sm transition-colors duration-200", loading ? "text-blue-500" : "text-neutral-500")}
            />
            <span
              className={cn(
